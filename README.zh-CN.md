...
- ⏳ 提供处理阶段提示，可取消导入或比对；取消比对后可使用已载入文档直接重试。
...
- 🖼️ 支持内嵌图片比对，按图片状态显示单图或双图预览，提供联动/独立缩放、旋转、翻转、还原及视觉相似度详情。
- 🧮 原生 Word 公式以 MathML 展示并参与比对，转换时保留支持的段落和表格对齐方式。
...
2. 上传或拖入基准文档与修订文档；也可以点击“加载示例”。两侧文档就绪后会自动开始比对。
...
内置示例为一份软件服务采购合同的两个版本，图表放在对应条款附近：保留不变的服务链路、随合同总额从 100 万元调整为 120 万元的付款计划、被删除的默认验收规则，以及新增的数据泄露处置义务。图中标明名称、单位和条款依据，并使用高清图片供放大查看。安装项目依赖与 Chrome 后，可运行 `python3 scripts/build_sample_images.py`，从合同数据重新生成这些图表。
