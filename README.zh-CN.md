...
5. 遇到已确认的干扰项时，可忽略该差异；已忽略项可从结果栏恢复。
6. 出现版面明细、相似差异推荐或表格提示时可进一步查看，也可以替换任意一侧文档继续比对。
...
- 原生 DOCX 页眉页脚会始终在解析时读取，用作版面线索，保留在版面明细中，并从最终展示正文中移除。
- 开启 `版面过滤` 后，转换后进入正文的页眉页脚如果命中页眉页脚线索，也可从比对输入中排除。
- 支持过滤 `第 1/5 页`、`第 1 页，共 5 页`、`第1页/共5页`、`共5页 第1页`、`1/5`、`Page 1 of 5`、`P. iv of x`、`页码：1`、`— 1/5 —` 等页码文本。
- 支持过滤重复出现的短版面文字，例如保密提示、版权声明、文件编号、电话或邮箱页脚。
- 为避免误删正文，电话、邮箱、联系人等片段线索只用于带页码特征的候选文本。
- 结果栏中的 `版面干扰` 数量可点击查看版面明细。
...
- 🧮 文本比对由 diff-match-patch 完成，运行在 Web Worker 中；长时间未完成的 worker 请求会超时，不会再回退到主线程继续卡住页面。
- 🧯 同步比对回退带有文本长度保护，避免不支持 worker 的环境在大文档上阻塞界面。
...
- 🙈 差异忽略状态和相似差异评分集中在 `src/utils/diffReview.ts`。
