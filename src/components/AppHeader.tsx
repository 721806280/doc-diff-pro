...
import { getThemeSwatchStyle, THEME_COLORS } from '@/utils/themeColor';
...
export default function AppHeader({
  ...
  const appearanceLabel =
    settings.appearanceMode === 'dark' ? i18n.header.switchToLightMode : i18n.header.switchToNightMode;
    ...
    <header className={`app-toolbar ${open ? 'app-toolbar--settings-open' : ''}`}>
      ...
      >
        <span className="brand-logo-glow" aria-hidden="true">
          <svg viewBox="0 0 32 32" fill="none">
            <g className="brand-logo__document brand-logo__document--accent">
              <rect
                className="brand-logo__page brand-logo__page--accent"
                x="3"
                y="3"
                width="12"
                height="26"
              ...
              />
              <path
                className="brand-logo__line brand-logo__line--accent"
                d="M6 9h6M6 13h6M6 17h5"
                strokeWidth="1.5"
                strokeLinecap="round"
              />
            </g>
            <g className="brand-logo__document brand-logo__document--revision">
              <rect
                className="brand-logo__page brand-logo__page--revision"
                x="17"
                y="3"
                width="12"
                height="26"
              ...
              />
              <path
                className="brand-logo__line brand-logo__line--revision"
                d="M20 9h6M20 13h6M20 17h5"
                strokeWidth="1.5"
                strokeLinecap="round"
              />
            </g>
        ...
        <span className="brand-text" aria-hidden="true">
          <span className="brand-title">
            DocDiff <span className="badge-pro">Pro</span>
          </span>
        </span>
      </button>
      ...
      <div ref={controlRef} className="header-actions">
        {canSwapDocuments && (
          <button
            type="button"
            className="toolbar-icon-button swap-documents-trigger"
        ...
        <div className={`settings-control ${open ? 'settings-control--open' : ''}`}>
          ...
          >
            <svg
              className="settings-sliders-icon"
              viewBox="0 0 24 24"
              aria-hidden="true"
              fill="none"
              stroke="currentColor"
            ...
            >
              <path d="M4.5 8h15" />
              <circle cx="9" cy="8" r="2.4" fill="var(--bg-panel-solid)" />
              <path d="M4.5 16h15" />
              <circle cx="15" cy="16" r="2.4" fill="var(--bg-panel-solid)" />
            </svg>
          </button>
            ...
            <div ref={popoverRef} className="settings-popover" role="dialog" aria-labelledby="compare-settings-title">
              <div className="settings-popover__header">
                ...
                {!usingDefaults && (
                  ...
                  >
                    <svg viewBox="0 0 24 24" aria-hidden="true" fill="none" stroke="currentColor" strokeWidth="2.25">
                      <path d="M7.2 7.8A6.9 6.9 0 1 1 5.9 13" />
                      <path d="M7.2 4.7v3.2H4" />
                      <path d="M12 8.9v3.8l2.5 1.5" />
                    </svg>
                ...
                <div className="granularity-segmented" role="radiogroup" aria-label={i18n.header.diffGranularityLabel}>
                  {(['semantic', 'word', 'char'] as const).map((value) => (
                    <button
                      ...
                      type="button"
                      role="radio"
                      className={`granularity-segmented__option ${settings.diffGranularity === value ? 'active' : ''}`}
                      aria-checked={settings.diffGranularity === value}
                      title={i18n.header.granularityOptions[value]}
                      onClick={() => updateSetting('diffGranularity', value)}
                    >
              ...
              <fieldset className="settings-section settings-section--framed">
                ...
                <div
                  className="settings-toggle-list settings-toggle-list--primary"
                  aria-label={i18n.header.compareRulesLabel}
                >
                  <SettingsToggle
                    label={i18n.header.ignoreSpaces}
                    title={i18n.header.ignoreSpacesTitle}
                    active={settings.ignoreSpaces}
                    onClick={() => updateSetting('ignoreSpaces', !settings.ignoreSpaces)}
                  />
                  <SettingsToggle
                    label={i18n.header.ignoreFullHalfWidth}
                    title={i18n.header.ignoreFullHalfWidthTitle}
                    active={settings.ignoreFullHalfWidth}
                    onClick={() => updateSetting('ignoreFullHalfWidth', !settings.ignoreFullHalfWidth)}
                  />
                  <SettingsToggle
                    label={i18n.header.filterLayoutNoise}
                    title={i18n.header.filterLayoutNoiseTitle}
                    active={settings.filterLayoutNoise}
                    onClick={() => updateSetting('filterLayoutNoise', !settings.filterLayoutNoise)}
                  />
                </div>
              </fieldset>
              ...
              <fieldset className="settings-section settings-section--framed settings-section--view">
                  ...
                  <div className="theme-color-swatches">
                    {THEME_COLORS.map((option) => (
                      <button
                        ...
                        type="button"
                        role="radio"
                        className={`theme-color-swatch ${settings.themeColor === option ? 'active' : ''}`}
                        style={getThemeSwatchStyle(option)}
                        aria-checked={settings.themeColor === option}
                        aria-label={i18n.header.themeColorOptions[option]}
                        title={i18n.header.themeColorOptions[option]}
                        onClick={() => updateSetting('themeColor', option)}
                      >
                ...
                <div
                  className="settings-toggle-list settings-toggle-list--plain"
                  aria-label={i18n.header.viewOptionsLabel}
                >
                  <SettingsToggle
                    label={i18n.header.showDiffMap}
                    title={i18n.header.showDiffMapTitle}
                    active={settings.showDiffMap}
                    onClick={() => updateSetting('showDiffMap', !settings.showDiffMap)}
                  />
                  <SettingsToggle
                    label={i18n.header.syncScroll}
                    title={i18n.header.syncScrollTitle}
                    active={settings.syncScroll}
                  ...
                  <SettingsToggle
                    ...
                    onClick={() => updateSetting('showTableHints', !settings.showTableHints)}
                  />
                  <SettingsToggle
                    label={i18n.header.enableDiffIgnore}
                    title={i18n.header.enableDiffIgnoreTitle}
                    active={settings.enableDiffIgnore}
                    onClick={() => updateSetting('enableDiffIgnore', !settings.enableDiffIgnore)}
                  />
                  {settings.enableDiffIgnore && (
                    <div className="settings-subgroup">
                      <div
                        className="similar-level-control"
                        role="radiogroup"
                      ...
                      >
                        ...
                        <div className="similar-level-segmented">
                          {SIMILAR_OPTIONS.map((option) => {
                            const active =
                              option === 'off'
                                ? !settings.enableSimilarDiffs
                                : settings.enableSimilarDiffs && settings.similarDiffLevel === option;
                            return (
                              <button
                                key={option}
                                type="button"
                                ...
                                title={
                                  option === 'off'
                                    ? i18n.header.enableSimilarDiffsOffTitle
                                    : i18n.header.similarDiffLevelTitle[option]
                                }
                                onClick={() => setSimilarOption(option)}
                              >
                                {option === 'off'
                                  ? i18n.header.enableSimilarDiffsOff
        ...
        <button
          type="button"
          className={`toolbar-icon-button appearance-trigger ${settings.appearanceMode === 'dark' ? 'active' : ''}`}
          aria-label={appearanceLabel}
          title={appearanceLabel}
          onClick={() => {
            closeSettings(false);
            updateSetting('appearanceMode', settings.appearanceMode === 'dark' ? 'light' : 'dark');
          }}
        >
          {settings.appearanceMode === 'dark' ? (
            <svg
              className="appearance-icon appearance-icon--sun"
              viewBox="0 0 24 24"
              aria-hidden="true"
              fill="none"
              stroke="currentColor"
            ...
            >
              ...
              <path d="M12 3.2v2.1M12 18.7v2.1M4.2 12h2.1M17.7 12h2.1M6.45 6.45l1.5 1.5M16.05 16.05l1.5 1.5M17.55 6.45l-1.5 1.5M7.95 16.05l-1.5 1.5" />
            </svg>
          ) : (
            <svg
              className="appearance-icon appearance-icon--moon"
              viewBox="0 0 24 24"
              aria-hidden="true"
              fill="none"
              stroke="currentColor"
              strokeWidth="1.9"
            >
              <path d="M20.45 14.65A8.2 8.2 0 0 1 9.35 3.55A8.65 8.65 0 1 0 20.45 14.65z" />
            </svg>
          )}
        </button>
        ...
        <button
          type="button"
          className="toolbar-icon-button language-trigger"
        ...
        >
          <svg
            className={`language-icon ${locale === 'en' ? 'is-en' : ''}`}
            viewBox="0 0 24 24"
            aria-hidden="true"
            fill="none"
          >
            <text className="language-icon__symbol language-icon__symbol--zh" x="1.2" y="12">
              中
            </text>
            <path className="language-icon__corner language-icon__corner--top" d="M16 4L19.5 4Q21 4 21 5.5L21 8" />
            <text className="language-icon__symbol language-icon__symbol--en" x="12.2" y="22">
              A
            </text>
            <path className="language-icon__corner" d="M8 20L4.5 20Q3 20 3 18.5L3 16" />
          </svg>
        </button>

        {showGithubLink && (
          <a
            className="toolbar-icon-button github-link"
  ...
  return (
    ...
    >
      ...
      <span className="settings-toggle__switch" aria-hidden="true">
        <span className="settings-toggle__switch-thumb" />
      </span>
    </button>
  );
}
