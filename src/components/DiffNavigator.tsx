...
export default function DiffNavigator({
  ...
  useEffect(() => {
    if (summary.layoutNoiseFiltered === 0) setLayoutOpen(false);
  }, [summary.layoutNoiseFiltered]);
      ...
      <div className="floating-navigator">
        <div className="navigator-summary">
          <div className="summary-strip">
            <span className="summary-chip similarity" title={i18n.diffNavigator.similarityTitle}>
            ...
            {summary.total > 0 && (
              <>
                <span className="summary-chip modified">
                  {i18n.diffNavigator.modified} <strong>{summary.modified}</strong>
                </span>
                <span className="summary-chip inserted">
                  {i18n.diffNavigator.inserted} <strong>{summary.inserted}</strong>
                </span>
                <span className="summary-chip deleted">
            ...
            {summary.layoutNoiseFiltered > 0 && (
              <button
                type="button"
                className={`summary-chip layout-noise ${layoutOpen ? 'active' : ''}`}
            ...
            {ignoredCount > 0 && (
              <button
                type="button"
                className={`summary-chip ignored ${ignoredOpen ? 'active' : ''}`}
        ...
        {summary.total > 0 && activeDiffCount > 0 ? (
          <div className="navigator-controls">
            ...
            >
              <div className="diff-progress-meta">
                <span className="diff-progress-index">
                  <span className="diff-progress-label">{i18n.diffNavigator.difference}</span>
                  <span className="diff-progress-count">
                    {activeDiffIndex}
              ...
              <div className="diff-progress-track">
                <div className="diff-progress-bar" style={{ width: `${progressPercent}%` }} />
              </div>
            </div>
            <div className="nav-triggers">
              <button
                className="btn-action-nav btn-action-nav--previous"
                title={i18n.diffNavigator.shortcutTitle(i18n.diffNavigator.previous, 'Alt+↑')}
                aria-keyshortcuts="Alt+ArrowUp"
                onClick={onPrevious}
                disabled={!canPrevious}
              >
                <span className="btn-action-nav__icon" aria-hidden="true">
                  <svg width="12" height="12" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="2.4">
                    <polyline points="15 18 9 12 15 6" />
                  </svg>
                </span>
                <span className="btn-action-nav__label">{i18n.diffNavigator.previous}</span>
              </button>
              <button
                className="btn-action-nav btn-action-nav--next"
                title={i18n.diffNavigator.shortcutTitle(i18n.diffNavigator.next, 'Alt+↓')}
                aria-keyshortcuts="Alt+ArrowDown"
                onClick={onNext}
                disabled={!canNext}
              >
                <span className="btn-action-nav__label">{i18n.diffNavigator.next}</span>
                <span className="btn-action-nav__icon" aria-hidden="true">
                  <svg width="12" height="12" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="2.4">
          ...
          <div className="ignored-empty">
            ...
            <button type="button" onClick={onRestoreAllIgnored}>
