...
export default function MobilePaneSwitch({
  activePane,
  i18n,
  onChange
}: {
  ...
  return (
    <div className="mobile-pane-switch" role="radiogroup" aria-label={i18n.documentPane.mobileViewLabel}>
      <button
        type="button"
        role="radio"
        className={`mobile-pane-switch__option is-original ${activePane === 'A' ? 'active' : ''}`}
      ...
      <button
        type="button"
        role="radio"
        className={`mobile-pane-switch__option is-revised ${activePane === 'B' ? 'active' : ''}`}
